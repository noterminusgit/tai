defmodule Tai.VenueAdapters.Bitmex.Positions do
  ...
  def positions(venue_id, credential_id, credentials) do
    ...
    with {:ok, venue_positions, _rate_limit} <-
           HTTPClient.get("/api/v1/position", venue_credentials) do
      positions =
        venue_positions
  ...
  defp build(%{"currentQty" => 0}, _, _), do: nil
  ...
  defp side(%{"currentQty" => qty}) when qty > 0, do: :long
  defp side(%{"currentQty" => qty}) when qty < 0, do: :short

  defp qty(%{"currentQty" => qty}) when qty > 0, do: Decimal.new(qty)
  defp qty(%{"currentQty" => qty}) when qty < 0, do: Decimal.new(-qty)

  defp entry_price(%{"avgEntryPrice" => p}), do: Tai.Utils.Decimal.cast!(p)

  defp leverage(%{"leverage" => l}), do: Tai.Utils.Decimal.cast!(l)

  defp margin_mode(%{"crossMargin" => true}), do: :crossed
  defp margin_mode(%{"crossMargin" => false}), do: :fixed
end
