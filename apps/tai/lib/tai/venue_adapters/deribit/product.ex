defmodule Tai.VenueAdapters.Deribit.Product do
  ...
  defp to_status(%{"is_active" => true}), do: :trading
  defp to_status(%{"is_active" => false}), do: :halt

  defp to_type(%{"kind" => "option"}), do: :option

  defp to_type(%{"kind" => "future", "instrument_name" => instrument_name}) do
    instrument_name
    |> String.split("-")
    |> Enum.reverse()
    |> case do
