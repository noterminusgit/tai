defmodule Tai.Events.StreamMessageUnhandledTest do
  ...
  test "struct has required fields" do
    ...
    event =
      struct!(Tai.Events.StreamMessageUnhandled,
        venue_id: :my_venue,
        msg: %{"type" => "unknown", "data" => "some data"},
        received_at: received_at
      )

    assert event.venue_id == :my_venue
    assert event.msg == %{"type" => "unknown", "data" => "some data"}
  ...
  test ".to_data/1 returns the struct as a map" do
    ...
    assert json.msg == %{"type" => "unknown", "data" => "some data"}
  end
end
